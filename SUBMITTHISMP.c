...
void MostWatchedMovie(SeatingData s[][6]);
...
int main() {
  ...
  int i;
  char d;
  char dChar[7] = {'1', '2', '3', '4', '5', '6', '7'};
  int dInt[7] = {1, 2, 3, 4, 5, 6, 7};
  ...
  while (exitflag == 1) {
    ...
    printf("[7.] Top 3 Movies\n");

    printf("< ");
    scanf(" %s", &d);
    decider = -1;

    for(i = 0; i < 7; i++){if(d == dChar[i]){decider = dInt[i];}}
...
void loadschedule(SeatingData s[][6], MovieData Update[]) {
  ...
  if (newfp == NULL) {
    ...
    for (i = 0; i < 6; i++) {
    fscanf(newfp, " %d", &Update[i].CinemaNumber);
    //printf(" %d\n", preload[i].CinemaNumber);
    fscanf(newfp, " %50[^\n]", Update[i].MovieTitle);
    fscanf(newfp, " %1000[^\n]", Update[i].MovieDescription);
    fscanf(newfp, " %50s", Update[i].RunningTime);
    for (j = 0; j < 6; j++) {
      if(getc(newfp) == EOF){
        for(k = j; k < 6; k++){
            Update[i+1].CinemaNumber = atoi(temp);
            //printf("i%d %d\n", i, preload[i].CinemaNumber);
            strcpy(Update[i].ShowTime[k], "NULLSHO");
        }
        j = 5;
      }else{
        fscanf(newfp, " %7s", temp);
        if(strlen(temp) > 1){
            strcpy(Update[i].ShowTime[j], temp);
...
void preloadmovie(MovieData preload[]) {
  ...
  int i, j, k;
  if (fp == NULL) {
    printf("Schedule File Missing! \n Enter Movie Schedule filename(Do not Include.txt)\n >");
    scanf(" %50[^\n]s",
...
/*ViewSchedule Displays the Movie Schedule
@param ScheduleView[] - Struct that contains all the data of the movies to be displayed
@returns void
*/
void ViewSchedule(MovieData ScheduleView[]) {
  int i, j;
...
/*MostWatchedMovie - Output the most watched movie of the current day.
@param s[][6] = Struct that contains the seatingdata of the customer
@returns void
Pre-condition:N/A.
*/
void MostWatchedMovie(SeatingData s[][6])
{
  ...
  for(i = 0; i < 6; i++)
  {
    MostWatched[i].Occupancy = 0;
    for(j = 0; j < 6; j++)
    {
      MostWatched[i].Occupancy += s[i][j].Occupancy;
      strcpy(MostWatched[i].Connect.MovieTitle, s[i][0].Connect.MovieTitle); 
    }
  } 
  ...
  for(i = 0; i < 6 - 1; i++)
  {
    ...
    if(min != i)
    {
      tempfr = MostWatched[i];
      MostWatched[i] = MostWatched[min];
      MostWatched[min] = tempfr;
    }
  }

printf("Most Watched Movies Today: \n");
  for(i = 0; i < 3; i++)
  {
    printf("%s\n %d\n\n", MostWatched[i].Connect.MovieTitle, MostWatched[i].Occupancy);
  }
}
